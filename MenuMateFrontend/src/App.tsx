...
export default function App() {
  ...
  const getMenuList = async () => {
    try {
      const res = await authAxios.get("/menu/getmenu");
  ...
  const tableNum = getRandomInteger(1, 10);
  return (
    <>
      {page === "login" && <Login GotoPage={GotoPage} />}
      {page === "main" && <MainDashboard menu={menu} GotoPage={GotoPage} />}
      {page === "home" && (
        <Main
